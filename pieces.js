class Piece {
  ...
  insert_position(line, column, board) {
    if (
      ...
      && (this.old_positionY !== line || this.old_positionX !== column)
    ) {
      this.positionY = line;
      this.positionX = column;
      return true;
    ...
    console.log(
      'Não foi possível mover a peça, fora do range de possibilidades',
    );
    return false;
  }
}
  ...
  calculate_choices(board) {
    ...
    if (this.side === 'black' && this.positionY === 1) {
      if (board.table[this.positionY + 1][this.positionX].name === 'nothing') {
        ...
        if (
          board.table[this.positionY + 2][this.positionX].name === 'nothing'
        ) {
...
class Knight extends Piece {
  calculate_choices(board) {
    ...
    if (this.positionY + 1 <= 7 && this.positionX + 2 <= 7) {
      if (
        board.table[this.positionY + 1][this.positionX + 2].side !== this.side
      ) {
    ...
    if (this.positionY + 2 <= 7 && this.positionX + 1 <= 7) {
      if (
        board.table[this.positionY + 2][this.positionX + 1].side !== this.side
      ) {
...
class Bishop extends Piece {
  set_choices(board, line, column) {
    ...
    // if finds enemy
    if (
      board.table[this.positionY][this.positionX].other_side
      === board.table[line][column].side
    ) {
      board.choices[line][column] = 1;
      return true;
    }
...
class King extends Piece {
  calculate_choices(board) {
    ...
    if (this.positionY - 1 >= 0 && this.positionX + 1 <= 7) {
      if (
        board.table[this.positionY - 1][this.positionX + 1].side !== this.side
      ) {
